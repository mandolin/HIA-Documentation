...
| W-P45 minimal remote execution or blocked result | `scripts/prepare-wp45-minimal-remote-execution-or-blocked-result.mjs` | `0.1.0-draft` |
| W-P45 closeout W-P46/W-P47 inputs | `scripts/prepare-wp45-closeout-wp46-wp47-inputs.mjs` | `0.1.0-draft` |
| W-P46 target-owner evidence ingestion intake | `scripts/prepare-wp46-target-owner-evidence-ingestion-intake.mjs` | `0.1.0-draft` |
...
| W-P45 minimal remote execution or blocked result | `main-repo` scripts | W-P45.6 evidence evaluating the final execution gate. Without explicit final network-send approval and host secret access it emits a review-only `execution-gate-blocked` result before any OpenAI Responses API destination is contacted. 中文：该 surface 当前走明确阻塞分支，不执行真实 provider API call。 |
| W-P45 closeout W-P46/W-P47 inputs | `main-repo` scripts | W-P45.7 evidence closing the controlled remote provider execution slice. It preserves the blocked-before-network result, records that no real provider API call occurred, and prepares W-P46 target-owner plus W-P47 checked-apply review-only inputs. 中文：该 closeout 只交接输入，不授予目标仓库或 checked apply 写入权。 |
| W-P46 target-owner evidence ingestion intake | `main-repo` scripts | W-P46.1 evidence defining the owner-provided evidence ingestion boundary. It accepts redacted target-owner metadata only, while HIA automation remains unable to create branches, open pull requests, run target commands, read target source bodies, trigger checked apply or mutate repositories. 中文：该 intake 是目标所有者证据接收边界，不是目标仓库操作授权。 |
...
- W-P45 minimal remote execution or blocked result evidence may either execute a minimal remote smoke after all explicit final gates pass, or produce an explicit blocked result. In the blocked path it must not contact provider destinations, read credential values, include request/response/source bodies, fake provider success, trigger checked apply, write workspaces, create direct edit objects or mutate target repositories.
- W-P45 closeout W-P46/W-P47 inputs evidence may summarize W-P45 results and prepare downstream inputs only. It must not reclassify blocked results as provider success, execute providers or target commands, grant final network-send approval, read credentials, trigger checked apply, write workspaces, create direct edits or mutate target repositories.
- W-P46 target-owner evidence ingestion intake may define owner-provided evidence kinds, required intake sections, redaction duties and host review projections only. It must not run target commands, create sandboxes, branches or pull requests, push commits, read target source bodies, execute providers, trigger checked apply, write workspaces, create direct edits or mutate target repositories.
