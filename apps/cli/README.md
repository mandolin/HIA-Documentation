...
- `hia --help`
- `hia docs build --input <file> --out <dir> [--locale <locale>] [--manifest <file>]`

The build command reads a HIA document JSON fixture, validates it through `@hia-doc/core`, reports diagnostics, renders it through `@hia-doc/renderer-html`, writes HTML/theme assets, and emits an output manifest. The default manifest path is `hia-manifest.json`.
