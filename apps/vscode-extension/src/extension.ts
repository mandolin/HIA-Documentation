...
import {
  ...
  type HiaCheckedApplySandboxConfirmationChoice,
  type HiaCheckedApplySandboxEvidenceSummary,
...
/**
 * 在 VS Code 中只读展示 W-P38 sandbox checked-apply 确认证据。
 * Show W-P38 sandbox checked-apply confirmation evidence read-only in VS Code.
 *
 * 中文：该命令只消费主仓生成的 synthetic sandbox evidence，不调用
 * VS Code 工作区应用编辑接口，不写入 workspace，也不修改目标项目。
 * English: This command only consumes synthetic sandbox evidence generated by
 * the main repo; it never calls the VS Code workspace edit-application API,
 * writes the workspace or mutates target repositories.
 */
async function showHiaCheckedApplySandboxConfirmation(outputChannel: vscode.OutputChannel): Promise<void> {
  const workspaceRoot = resolveWorkspaceRoot();

  if (!workspaceRoot) {
    void vscode.window.showWarningMessage("Open the HIA main repo workspace before showing checked apply sandbox confirmation.");
    return;
  }

  const evidencePath = path.join(workspaceRoot, "dist", "wp38-host-owned-writable-apply-sandbox", "evidence.json");
  let evidence: HiaCheckedApplySandboxEvidenceSummary;

  try {
    evidence = JSON.parse(await readFile(evidencePath, "utf8")) as HiaCheckedApplySandboxEvidenceSummary;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    outputChannel.show(true);
    outputChannel.appendLine(`HIA checked apply sandbox evidence could not be read: ${message}`);
    void vscode.window.showWarningMessage("Run pnpm run wp38:writable-sandbox:evidence before showing sandbox confirmation.");
    return;
  }
  ...
  outputChannel.show(true);
  outputChannel.appendLine("HIA checked apply sandbox confirmation evidence:");

  if (choices.length === 0) {
    for (const line of createHiaCheckedApplySandboxConfirmationReport(evidence)) {
      outputChannel.appendLine(`- ${line}`);
    }

    void vscode.window.showWarningMessage("No checked apply sandbox transaction is available. See HIA output.");
    return;
  }

  const selected = await vscode.window.showQuickPick<CheckedApplySandboxConfirmationQuickPickItem>(
    choices.map((choice) => ({
      ...choice,
      choice
    })),
    {
      placeHolder: "Choose a W-P38 checked apply sandbox confirmation"
    }
  );
  ...
  outputChannel.appendLine(`Selected sandbox confirmation: ${selected.choice.label}`);

  for (const line of createHiaCheckedApplySandboxConfirmationReport(evidence, selected.choice.transaction)) {
    outputChannel.appendLine(`- ${line}`);
  }
