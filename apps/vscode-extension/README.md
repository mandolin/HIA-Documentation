...
`HIA: Build Docs` delegates to the shared CLI and reads `hia.build.*` workspace settings. `HIA: Open Preview` reads the generated manifest when available, opens the manifest entrypoint, and warns when the preview may be stale. `HIA: Validate Workspace` asks the LSP server for capability, resource index, authoring location and resource action data, then writes a validation report to the HIA output channel.
