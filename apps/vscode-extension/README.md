...
The extension also exposes the shared `hia/projectRelationGraph` request constant for the next visible relation-navigation slice. The command UI for that request is intentionally deferred until the host workflow can consume the LSP view model without parsing generated HTML.
