...
- `@hia-doc/lsp`: LSP 服务、diagnostics、resource index、capability、authoring location、resource action/preflight、completion、hover、definition 和 folding，只消费 core document。
...
- `@hia-doc/vscode-extension`: VS Code 开发版插件壳，激活 `.hia.json` 文件，通过 `vscode-languageclient` 启动 LSP，并提供 Build Docs、Open Preview、Validate Workspace、related-location quick fix 和 resource action quick fix。
