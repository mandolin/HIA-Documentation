...
- `docs/enterprise-baseline-current-owner-workflow-contract.md`: 企业 owner baseline/current 组合、deferred 语义与零目标权限边界。
...
- `packages/plugin-sdk`: documentation producer descriptor/request/result contract、schema 和 runtime helper。
...
- `@hia-doc/plugin-sdk`: 定义 `documentation-producer@0.1.0-draft`，校验并执行显式加载的单次 producer module；不负责自动发现、进程调度或 watch graph。
...
CLI 单文档生成物包含 `index.html`、`assets/hia-default.css`、`assets/hia-default.js` 和 `hia-manifest.json`；project 模式额外输出 `project-index.json`，供静态 portal 与搜索消费。端到端测试会检查这些文件以及生成物中不泄露本机绝对路径。
